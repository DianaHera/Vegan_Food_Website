window.addEventListener("load",function(){
	...
	if (prod_sel){
		...
		fetch("/produse_cos", {		
			...
			mode: 'cors',		
			cache: 'default',
			body: JSON.stringify({ //body e prop
				a:10,
				b:20,
        ...
        //raspunsul de la fetch care poate sa fie text sau json
		...
		.then(function(objson) {
			...
			for (let prod of objson){//se creaza element si alegem ce detalii vrem sa afisam 
				let article=document.createElement("article");
				article.classList.add("cos-virtual");
				var h2=document.createElement("h2");//am creat un tag h2 adica titlu
		...
		document.getElementById("cumpara").onclick=function(){
			...
			if (prod_sel){
				var vect_ids=prod_sel.split(",");//preluam id urile din cos
				fetch("/cumpara", {		
