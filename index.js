...
client.query("select * from unnest(enum_range(null::categ_alimente))", function (err, rez) {
    console.log(rez);
})
...
obGlobal.bdMongo = obGlobal.clientMongo.db('gfbd');
...
client.query("select * from unnest(enum_range(null::tipuri_alimente))", function (err, rezCategorie) {
    if (err) {
        console.log(err);
    }
    else {
...
app.get("/produse", function (req, res) {
    ...
    var conditieQuery = "";
    if (req.query.tip) {
        conditieQuery = ` where tip_aliment='${req.query.tip}'`
    }
    client.query("select * from unnest(enum_range(null::categ_alimente))", function (err, rezOptiuni) {

        client.query(`select * from alimente ${conditieQuery}`, function (err, rez) {
            if (err) {
                console.log(err);
                afisareEroare(res, 2);
            }
...
app.get("/produs/:id", function (req, res) {
    client.query(`select * from alimente where id=${req.params.id}`, function (err, rez) {
        if (err) {
            console.log(err);
            afisareEroare(res, 2);
        }
...
app.post("/produse_cos", function (req, res) {
    ...
    if (req.body.ids_prod.length != 0) {
        ...
        AccesBD.getInstanta().select({ tabel: "alimente", campuri: "nume,descriere,pret,gramaj,imagine".split(","), conditiiAnd: [`id in (${req.body.ids_prod})`] },
            function (err, rez) {
                if (err)
                    res.send([]);
                else
...
client.query("select id from alimente", function (err, rez) {
    if (err) {
